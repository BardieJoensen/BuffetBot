    ...
    *,
    ...
    if top10:
        ...
        for rank, u in enumerate(top10, 1):
            ticker = u["ticker"]
            alert  = next((a for a in all_alerts if a["ticker"] == ticker), None)
    ...
    haiku_tickers: set[str] = set()
    for u in universe:
        # A stock is haiku-screened if it has a haiku result (infer from tier history or deep_analyses)
        pass  # covered by analyzed_tickers superset

    out.append(f"  Universe total:       {total_universe} stocks")
    out.append(f"  Deep-analyzed:        {len(analyzed_tickers)} stocks ({len(analyzed_tickers)/max(total_universe,1):.1%})")
    out.append("")
    ...
    if positions:
        ...
        out.append(f"  {'Ticker':<6}  {'Tier':<4}  {'Stage':<5}  {'Entry':>8}  {'Current':>8}  {'P&L':>10}  {'P&L%':>7}")
        out.append(f"  {'------':<6}  {'----':<4}  {'-----':<5}  {'--------':>8}  {'--------':>8}  {'----------':>10}  {'-------':>7}")
...
def _text_s_tier(
    ...
    tier_reason = da.get("investment_thesis", "")[:100] if da else ""
    ...
    if thesis:
        ...
        # Wrap at 66 chars
        words, current_line = thesis[:500].split(), []
        for word in words:
...
""")
    ...
    if b_approaching:
        ...
        parts.append(
            "<table><tr><th>Ticker</th><th>Company</th><th>Price</th>"
            "<th>Target</th><th>Gap</th><th>Moat</th></tr>"
        )
        for a in b_approaching:
            ticker = a["ticker"]
            u      = universe_by_ticker.get(ticker, {})
        ...
        parts.append(
            '<p style="font-size:.85rem;color:#888">No B-tier stocks within 10% of target entry.</p>'
        )
    parts.append("</section>")
    ...
    if top10:
        ...
        parts.append(
            "<table><tr><th>#</th><th>Ticker</th><th>Company</th>"
            "<th>Score</th><th>Tier</th><th>Gap to Target</th></tr>"
        )
        for rank, u in enumerate(top10, 1):
            ticker = u["ticker"]
            alert  = next((a for a in all_alerts if a["ticker"] == ticker), None)
            ...
            tier_html  = (
                f'<span class="tier-badge" style="background:{tier_color}">{e(tier)}</span>'
                if tier != "—" else "—"
            )
            parts.append(
    ...
    if news_items:
        for item in news_items:
            ...
            parts.append(
                f'<div><strong>{e(ticker)}</strong> '
                f'<span style="color:#888;font-size:.8rem">{e(ts)}</span>'
            )
            if event_t:
    ...
    if sector_total:
        ...
        parts.append('<div class="bar-row" style="font-weight:600;font-size:.78rem">'
    ...
    if cap_total:
        ...
        for cap_label, cap_key in [("Large ($10B+)", "large"), ("Mid ($1-10B)", "mid"), ("Small (<$1B)", "small")]:
            ...
            pct      = analyzed / total if total else 0
            parts.append(
                f"<tr><td>{e(cap_label)}</td><td>{total}</td>"
                f"<td>{analyzed}</td><td>{pct:.0%}</td></tr>"
            )
        parts.append("</table>")
    parts.append("</section>")
    ...
    if c_alerts:
        ...
        parts.append(
            '<p style="font-size:.85rem;color:#888;margin-bottom:10px">'
            "Re-evaluate next scheduled cycle.</p>"
        )
...
def _html_s_card(
    ...
    lines = [f'<div class="card s-tier">']
    lines.append(f'<h3>{e(ticker)}: {e(name)}</h3>')
    lines.append(f'<span class="tier-badge s">S-TIER</span>&nbsp;'
    ...
    if staged:
        ...
        if entry_strs:
            lines.append(f'<div class="staged"><strong>Staged Entry:</strong> '
                         + "&nbsp;&nbsp;|&nbsp;&nbsp;".join(entry_strs) + "</div>")
    ...
    risks = (da.get("key_risks") or []) if da else []
    if risks:
        lines.append("<details><summary>Key Risks</summary>"
                     "<ul style='font-size:.85rem;margin:6px 0 0 18px'>")
        for r in risks[:3]:
    ...
    if breakers:
        lines.append("<details><summary>Thesis Breakers (sell triggers)</summary>"
...
def _html_a_card(
    ...
    lines = [f'<div class="card a-tier">']
    lines.append(f'<h3>{e(ticker)}: {e(name)}</h3>')
    lines.append(f'<span class="tier-badge a">A-TIER</span>&nbsp;'
                 f'<span style="font-size:.82rem;color:#666">Moat: {e(moat)} | Conviction: {e(conviction)}</span>')

    if entry_strs:
        lines.append(f'<div class="staged"><strong>Entry Plan:</strong> '
                     + "&nbsp;&nbsp;|&nbsp;&nbsp;".join(entry_strs) + "</div>")
    ...
    if thesis:
        lines.append(
            f"<details><summary>Investment Thesis</summary>"
            f'<div class="thesis-box">{e(thesis[:400])}</div></details>'
        )
