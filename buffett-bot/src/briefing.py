...
class BriefingGenerator:
    ...
    def _generate_html(
        ...
        buy_candidates = sorted(
            ...
            key=lambda x: x.valuation.margin_of_safety or 0, reverse=True
        )
        watchlist = sorted(
...
<div class="summary-grid">""")
        parts.append(f'<div class="summary-card"><div class="num">{len(briefings)}</div><div class="label">Analyzed</div></div>')
        parts.append(f'<div class="summary-card"><div class="num" style="color:#4CAF50">{len(buy_candidates)}</div><div class="label">Buy</div></div>')
        parts.append(f'<div class="summary-card"><div class="num" style="color:#FF9800">{len(watchlist)}</div><div class="label">Watchlist</div></div>')
        parts.append(f'<div class="summary-card"><div class="num" style="color:#F44336">{len(bubble_warnings) if bubble_warnings else 0}</div><div class="label">Bubble Watch</div></div>')
        parts.append(f'<div class="summary-card"><div class="num">{len(radar_stocks) if radar_stocks else 0}</div><div class="label">Radar</div></div>')
  ...
  <div class="portfolio-stat"><div class="val {gain_class}">{gain_sign}${gain:,.0f} ({gain_sign}{gain_pct:.1%})</div><div class="lbl">Gain/Loss</div></div>
            ...
            if exposure:
                ...
                for sector, pct in sorted(exposure.items(), key=lambda x: x[1], reverse=True):
                    ...
                    parts.append(f'<div class="bar-row"><span class="bar-label">{e(sector)}</span><div class="bar-track"><div class="bar-fill" style="width:{width}%"></div></div><span class="bar-pct">{pct:.0%}</span></div>')
                parts.append("</div>")

            warnings = portfolio_summary.get('sector_warnings', [])
            if warnings:
                parts.append('<div style="margin-top:12px;color:#F44336;font-weight:600">Concentration Warnings:</div><ul style="margin:4px 0 0 20px;font-size:.9rem">')
                for w in warnings:
        ...
        # Radar
        if radar_stocks:
            parts.append('<section><h2>Radar</h2><p style="font-size:.9rem;color:#666;margin-bottom:12px">Passed screening, not yet analyzed.</p><div class="radar-grid">')
...
<footer>Buffett Bot &middot; {e(month_str)}</footer>
    ...
    def _html_stock_card(self, briefing: StockBriefing, card_type: str) -> str:
        ...
        if card_type == "buy" and briefing.position_size:
            ...
            lines.append(f'<div class="sizing"><strong>Position Sizing ({e(str(sz.get("conviction", "MEDIUM")))} conviction):</strong> '
        ...
        # Thesis
        lines.append(f"<details><summary>Investment Thesis</summary><p style='font-size:.9rem;margin-top:8px'>{e(briefing.analysis.investment_thesis[:600])}</p></details>")
        ...
        if briefing.analysis.thesis_risks:
            lines.append("<details><summary>Thesis-Breaking Events</summary><ul style='font-size:.9rem;margin:8px 0 0 20px;color:#d32f2f'>")
    ...
    def _html_bubble_card(self, warning) -> str:
        ...
        lines = [f'<div class="stock-card bubble">']
        ...
        if signals:
            lines.append("<details open><summary>Warning Signals</summary><ul style='font-size:.9rem;margin:8px 0 0 20px;color:#d32f2f'>")
